...
class CWRUFFT_inconsistent(object):
    ...
    def data_split(self, transfer_learning=True):
        if transfer_learning:
            name_source, name_target, label_source, label_target, num_classes = dataset_information(
                self.source_N, self.target_N, self.inconsistent)

            # get source train and val
            list_data = get_files(
                self.data_dir, self.source_N, name_source, label_source)
            ...
            list_data = get_files(
                self.data_dir, self.target_N, name_target, label_target)
            data_pd = pd.DataFrame(
                {"data": list_data[0], "label": list_data[1]})
            train_pd, val_pd = train_test_split(
                data_pd, test_size=0.2, random_state=40, stratify=data_pd["label"])
